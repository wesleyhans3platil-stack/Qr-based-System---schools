...
$consecutive_sql = "SELECT s.lrn, s.name, s.id as student_id, s.created_at, s.active_from, s.school_id, gl.name as grade, sec.name as section, sch.name as school_name
        FROM students s
...
$r = $conn->query($consecutive_sql . " ORDER BY sch.name, gl.name, sec.name, s.name LIMIT 200");
if ($r) {
...
for ($count = 0; $count < 7; $count++) {
    ...
    $day_total = 0;
    $r2 = $conn->query("SELECT COUNT(*) as cnt FROM students WHERE status='active' AND DATE(created_at) <= '$d'");
    if ($r2) $day_total = $r2->fetch_assoc()['cnt'];
    array_unshift($div_trend, ['date' => date('M d', strtotime($d)), 'present' => $cnt, 'absent' => max(0, $day_total - $cnt)]);
    $d = date('Y-m-d', strtotime($d . ' -1 day'));
}
...
if ($view_school) {
    ...
    // Weekly school trend (only school days)
    $d2 = $filter_date;
    for ($count = 0; $count < 7; $count++) {
        ...
        if ($r2) $cnt = $r2->fetch_assoc()['cnt'];
        $day_total = 0;
        $r2 = $conn->query("SELECT COUNT(*) as cnt FROM students WHERE status='active' AND school_id = $vs AND DATE(created_at) <= '$d2'");
        if ($r2) $day_total = $r2->fetch_assoc()['cnt'];
        array_unshift($drill_trend, ['date' => date('M d', strtotime($d2)), 'present' => $cnt, 'absent' => max(0, $day_total - $cnt)]);
        $d2 = date('Y-m-d', strtotime($d2 . ' -1 day'));
    }
}
?>
    ...
    <div class="main-content">
        ...
        <div class="top-bar">
            <div class="page-header">
                <a href="sds_dashboard.php?date=<?= $filter_date ?>" class="btn btn-outline btn-sm" style="margin-bottom:12px;">
        ...
        <div class="top-bar">
            <div class="page-header">
                ...
                <p>Schools Division Superintendent — Division-Level Monitoring</p>
            </div>
            <div class="top-bar-right">
