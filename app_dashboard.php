...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent}
        ...
        .content{transition:opacity .25s ease, transform .3s ease;will-change:transform,opacity;}
        .content.fade-out{opacity:.25;transform:translateX(-12px);}
        .content.fade-in{opacity:1;transform:translateX(0);}
        ...
        .nav-item{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;font-size:.55rem;font-weight:600;color:var(--on-surface-v);text-decoration:none;padding:0 8px;border:none;background:none;cursor:pointer;position:relative;flex:1;transition:color .2s, transform .12s;touch-action:manipulation;-webkit-user-select:none;user-select:none;-webkit-touch-callout:none;-webkit-tap-highlight-color:transparent;outline:none;}
...
<body>
    ...
    async function pollData() {
        ...
        try {
            ...
            const s = data.stats;
            const hash = JSON.stringify({ stats: s, trend: data.trend });
            if (hash === lastPollHash) {
                // No changes since last poll; avoid DOM updates to prevent stutter.
                isPolling = false;
                return;
            }
            lastPollHash = hash;
